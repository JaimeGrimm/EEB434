...
### 5.1 Calculating summary statistics for environmental variables

If we are interested in generating some summary statistics (including mean and standard deviation), there are a couple of ways we can do that in R:

```{r}
#First, we could simply call the summary() function for the variable we are interested in. This function gives us the minimum, maximum, mean, median and quantile values of our data:
summary(env$temp)

#Alternatively, we could caluculate the mean using the mean() function:
mean(env$temp)

#If we're interested in the standard deviation, we can calculate that using the sd() function:
sd(env$temp)
```
